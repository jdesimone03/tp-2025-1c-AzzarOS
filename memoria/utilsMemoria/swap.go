...
func EscribirProcesoEsSwap(proceso structs.ProcesoEnSwap) {
	...
	if error != nil {
		logueador.Error("Error al convertir el proceso a JSON")
		return
	}
	_, err = file.Write(append(dataJSON, '\n')) // Agrego salto de linea para que esten uno abajo del otro
	if err != nil {
		logueador.Error("Error al escribir el proceso en el archivo SWAP: %v", err)
		return
	}
...
func BuscarProcesoEnSwap(pid uint) *structs.ProcesoEnSwap {
	...
	if err := scanner.Err(); err != nil {
		logueador.Error("Error al leer el archivo SWAP: %v", err)
	}
	...
	for scanner.Scan() {
		...
		if err != nil {
			logueador.Error("Error al deserializar el proceso desde el archivo SWAP: %v", err)
	...
	if procesoEncontrado == nil {
		logueador.Error("No se encontró el proceso con PID %d en SWAP", pid)
		return nil
	}

	err = os.WriteFile(pathCorrecto, []byte{}, 0644) // Limpiar el archivo SWAP
	if err != nil {
		logueador.Error("Error al limpiar el archivo SWAP: %v", err)
		return nil
	}
...
func AcomodarProcesoEnMemoria(procesoEnSwap *structs.ProcesoEnSwap, pid uint) {
	...
	for i := range procesoEnSwap.Paginas {
...
func SwapInProceso(pid uint) {
	...
	if len(paginas) == 0 {
		logueador.Warn("No se encontraron páginas para el proceso %d", pid)
		return
	}
